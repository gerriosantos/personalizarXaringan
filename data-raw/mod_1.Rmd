...
dados <- haven::read_dta('DadosISS_bimestral.dta') %>%
  dplyr::mutate(Bimestre = dplyr::case_when(
    Bimestre == 1 ~ '1˚ Bim',
    Bimestre == 2 ~ '2˚ Bim',
    Bimestre == 3 ~ '3˚ Bim',
    Bimestre == 4 ~ '4˚ Bim',
  ...
  dplyr::select(Municipio, Bimestre, 'ISS por Habitante 2019' = iss19, 'ISS por Habitante 2020' = iss20) %>%
  tidyr::pivot_longer(cols = !Municipio:Bimestre, names_to = 'iss', values_to = 'iss_pop') %>%
  dplyr::filter(Municipio %in% c('Coreaú-CE', 'Sobral-CE'))
